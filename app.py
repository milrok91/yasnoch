...
def moon_info(date_local: dt.date, tz: ZoneInfo):
    ...
    try:
        rise = amoon.moonrise(loc.observer, date=date_local, tzinfo=tz)
    except Exception:
        rise = None
    try:
        set_ = amoon.moonset(loc.observer, date=date_local, tzinfo=tz)
    except Exception:
        set_ = None
    try:
        next_set = amoon.moonset(loc.observer, date=(date_local + dt.timedelta(days=1)), tzinfo=tz)
    except Exception:
        next_set = None
    intervals = []
    if rise and set_:
        if rise < set_:
            ...
            intervals.append((rise, date_local + dt.timedelta(days=1)))
            if next_set:
                intervals.append((date_local + dt.timedelta(days=1), next_set))
    elif rise and not set_:
        intervals.append((rise, date_local + dt.timedelta(days=1)))
    elif set_ and not rise:
        intervals.append((date_local, set_))
...
def filter_by_moon(averaged: Dict[int, Dict[str, float]], dusk: dt.datetime, dawn: dt.datetime, tz: ZoneInfo) -> Dict[int, Dict[str, float]]:
    ...
    if illum <= MOON_MAX_ILLUM or not intervals:
        return averaged
    blocks = []
    for a,b in intervals:
        s = max(a, dusk); e = min(b, dawn)
        if s < e:
            blocks.append((int(s.timestamp()), int(e.timestamp())))
    if not blocks:
        return averaged
    filtered = {}
    for ts, v in averaged.items():
        if any(a <= ts < b for a,b in blocks):
            continue
        filtered[ts] = v
    return filtered
...
def fmt_report(date_local: dt.date, dusk: dt.datetime, dawn: dt.datetime,
    ...
    illum, age, m_intervals = moon_info(dusk.date(), tz)
    ...
    if m_intervals:
        ...
        for a,b in m_intervals:
            s = max(a, dusk); e = min(b, dawn)
            if s < e:
                spans.append(f"{s.strftime('%H:%M')}–{e.strftime('%H:%M')}")
        if spans:
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "/moonfilter <0|1> — выкл/вкл учёт Луны (фильтрация часов, когда яркая Луна над горизонтом)\n"
    )
